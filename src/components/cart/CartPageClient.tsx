    ...
    <main className="container mx-auto px-4 py-10 sm:py-14 max-w-6xl">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <AnimatePresence mode="popLayout">
            {items.map((item) => (
              <motion.div
                ...
                className="flex gap-4 p-4 bg-card border rounded-xl shadow-xs hover:shadow-sm transition-shadow"
              >
                {/* Image */}
                <Link
                  href={`/product/${item.productId}`}
                  className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-lg overflow-hidden shrink-0 bg-muted"
                >
                  <Image
                    src={item.image}
                    alt={item.name}
        ...
        <div className="lg:col-span-1">
          <div className="bg-card border rounded-xl p-6 shadow-sm sticky top-20">
